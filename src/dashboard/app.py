...
def show_overview(data: Dict):
    ...
    with col1:
        st.metric(
            label="👥 Utilisateurs",
            value=f"{len(users_df):,}",
            delta="Profils générés"
        )

    with col2:
        st.metric(
            label="📉 Taux de pauvreté",
            value=f"{poverty_rate:.1%}",
            delta="Q1 + Q2"
        )
    ...
    if not users_df.empty and "region" in users_df.columns:
        ...
        with col2:
            st.dataframe(region_summary.round(1), use_container_width=True, hide_index=True)
...
def show_poverty_overview(poverty_stats: pd.DataFrame):
    ...
    poverty_rate = poverty_stats['is_poor'].mean() if 'is_poor' in poverty_stats.columns else 0
    ...
    if 'wealth_index' in poverty_stats.columns:
        ...
        fig.add_vline(x=median_val, line_dash="dash",
                      annotation_text=f"Médiane: {median_val:.2f}", line_color="red")
        st.plotly_chart(fig, use_container_width=True)
...
def show_poverty_map(poverty_stats: pd.DataFrame):
    ...
    regional_stats = poverty_stats.groupby('region').agg({
        'wealth_index': ['mean', 'std'],
        'user_id': 'count',
        'is_poor': 'mean'
    }).reset_index()

    regional_stats.columns = ['Région', 'Indice moyen', 'Écart-type', 'Population', 'Taux pauvreté']
    ...
    with col2:
        ...
        display_df = regional_stats[['Région', 'Indice moyen', 'Taux pauvreté (%)', 'Population']].copy()
    ...
    if 'latitude' in poverty_stats.columns and 'longitude' in poverty_stats.columns:
        ...
        region_coords = poverty_stats.groupby('region').agg({
            'latitude': 'mean',
            'longitude': 'mean',
            'wealth_index': 'mean',
            'user_id': 'count'
        ...
        fig = px.scatter_mapbox(
            ...
            color='wealth_index',
            hover_name='region',
            hover_data={'wealth_index': ':.3f', 'user_id': True, 'latitude': False, 'longitude': False},
            color_continuous_scale='RdYlGn',
    ...
    with col1:
        ...
        poorest = regional_stats.nsmallest(5, 'Indice moyen')[['Région', 'Indice moyen', 'Taux pauvreté (%)']]
        st.dataframe(poorest, hide_index=True)

    with col2:
        st.markdown("**🟢 Régions les plus riches**")
        richest = regional_stats.nlargest(5, 'Indice moyen')[['Région', 'Indice moyen', 'Taux pauvreté (%)']]
...
def show_poverty_details(poverty_stats: pd.DataFrame):
    ...
    numeric_cols = ['wealth_index', 'recharge_amount_fcfa', 'mobility_radius_km',
                    'contact_diversity_score', 'call_duration_sec', 'data_mb']
    ...
    st.download_button(
        ...
        data=csv,
        file_name="poverty_analysis_results.csv",
        mime="text/csv"
    )
...
def show_congestion_analysis(mobility_df: pd.DataFrame):
    ...
    mobility_df['free_flow_speed'] = mobility_df['transport_mode'].map(free_flow_speeds).fillna(30)
    mobility_df['congestion_index'] = mobility_df['free_flow_speed'] / mobility_df['speed_kmh'].clip(lower=1)
    mobility_df['congestion_index'] = mobility_df['congestion_index'].clip(upper=5)  # Limiter à 5x
    
    # ===== HEURES DE POINTE =====
    st.markdown("### ⏰ Heures de Pointe")
    
    hourly_stats = mobility_df.groupby('hour_of_day').agg({
        'trip_id': 'count',
        'speed_kmh': 'mean',
        'congestion_index': 'mean',
        'duration_min': 'mean'
    }).reset_index()
    hourly_stats.columns = ['Heure', 'Nb trajets', 'Vitesse moy.', 'Indice congestion', 'Durée moy.']
    ...
    with col1:
        ...
        colors = ['#FF6B00' if h in peak_hours else '#FFA500' for h in hourly_stats['Heure']]
        ...
        fig.add_vrect(x0=6.5, x1=9.5, fillcolor="red", opacity=0.1, 
                      annotation_text="Pointe matin", annotation_position="top left")
        fig.add_vrect(x0=16.5, x1=19.5, fillcolor="red", opacity=0.1,
    ...
    with col2:
        ...
        # Pointe du matin
        morning_peak = hourly_stats[(hourly_stats['Heure'] >= 6) & (hourly_stats['Heure'] <= 10)]
        if not morning_peak.empty:
            peak_morning_hour = morning_peak.loc[morning_peak['Nb trajets'].idxmax(), 'Heure']
            st.metric("🌅 Pointe matin", f"{int(peak_morning_hour)}h00")
        
        # Pointe du soir
        evening_peak = hourly_stats[(hourly_stats['Heure'] >= 16) & (hourly_stats['Heure'] <= 20)]
        if not evening_peak.empty:
            peak_evening_hour = evening_peak.loc[evening_peak['Nb trajets'].idxmax(), 'Heure']
        ...
        st.metric("🚨 Plus congestionnée", f"{int(most_congested['Heure'])}h00",
        ...
        st.metric("✅ Plus fluide", f"{int(least_congested['Heure'])}h00",
    ...
    if 'locality' in mobility_df.columns:
        ...
        zone_stats.columns = ['Localité', 'Nb trajets', 'Vitesse moy.', 'Indice congestion', 'Durée moy.', 'Distance moy.']
        ...
        display_df['Vitesse moy.'] = display_df['Vitesse moy.'].round(1).astype(str) + ' km/h'
        display_df['Indice congestion'] = display_df['Indice congestion'].round(2)
        display_df['Durée moy.'] = display_df['Durée moy.'].round(1).astype(str) + ' min'
    ...
    if 'locality' in mobility_df.columns:
        ...
        st.info("💡 **Lecture** : Plus la couleur est rouge, plus la zone est congestionnée à cette heure. Vert = fluide.")
...
def show_mobility_details(mobility_df: pd.DataFrame):
    ...
    if "hour_of_day" in mobility_df.columns:
        ...
        hourly.columns = ["Heure", "Nombre de trajets", "Distance moyenne", "Durée moyenne"]
    ...
    if "transport_mode" in mobility_df.columns:
        mode_stats = (
            mobility_df.groupby("transport_mode")
            .agg({
...
def show_interactive_map(data: Dict):
    ...
    viz_type = st.selectbox(
        ...
        ["Densité par localité", "Heatmap", "Points individuels"]
    )
    ...
    if viz_type == "Densité par localité":
        ...
        fig = px.density_mapbox(
            ...
            center={"lat": 7.54, "lon": -5.55},
            zoom=5.5,
            title="Heatmap de la densité de population"
        )
        fig.update_layout(height=600, margin={"r": 0, "t": 30, "l": 0, "b": 0})
        st.plotly_chart(fig, use_container_width=True)
...
def main():
    ...
    st.sidebar.markdown("---")
    st.sidebar.caption("© 2024 - DataLab ANStat")
